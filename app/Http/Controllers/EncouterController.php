...
class EncouterController extends VController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        // Find data detail
        $encounterDetail = EncouterDetail::select('id', 'type', 'key', 'value')->whereEncouterId($id)->get()->toJson();

        if (view()->exists("voyager::$slug.edit-add")) {
            $view = "voyager::$slug.edit-add";
        }

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'encounterDetail'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $insert = $request->except('_token', 'options', 'start_at');
        $insert['start_at'] = Carbon::parse($request->start_at);
        $insert['options'] = $request->settings !== 'manual' ? json_encode($request->options) : null;
        $data->update($insert);
        
        
        if ($request->settings !== 'manual') {
            EncouterDetail::where('encouter_id',$data['id'])->delete();
            $listSoal = $data->autoChoseUjian($request->options, $data->mata_diklat_id);
            
            foreach ($listSoal as $kls => $vls) {
                foreach ($vls as $klsd => $vlsd) {
                    
                    EncouterDetail::create([
                        'encouter_id' => $data['id'],
                        'value' => $vlsd['soal'],
                        'key' => 'soal',
    ...
    public function relation(Request $request)
    {
        ...
        if ($request->type == 'get_by_diklat') {
            ...
            foreach ($rows as $key => $row) {
                ...
                // get mata diklat
                $total_count = Diklat::findOrFail($request->diklat_id)->mataDiklat()->count();
                $relationshipOptions = Diklat::findOrFail($request->diklat_id)->mataDiklat;

                $results = [];
                foreach ($relationshipOptions as $relationshipOption) {
                    $results[] = [
                        'id'   => $relationshipOption->id,
                        'text' => $relationshipOption->title,
                    ];
                }

                // get angkatan
                $total_countDetail = Diklat::findOrFail($request->diklat_id)->diklatDetail()->count();
                $relationshipOptionsDetail = Diklat::findOrFail($request->diklat_id)->diklatDetail;

                $resultsDetail = [];
                foreach ($relationshipOptionsDetail as $relationshipOptionDetail) {
                    $resultsDetail[] = [
                        'id'   => $relationshipOptionDetail->id,
                        'text' => $relationshipOptionDetail->title,
                    ];
                }
                ...
                return response()->json([
                    ...
                    'pagination_detail' => [
                        'more' => ($total_countDetail > ($skip + $on_page)),
                    ],
                    'results_mata'    => $results,
                    'pagination_mata' => [
    ...
    public function addBankSoal(Request $request, $id)
    {
        ...
        EncouterDetail::where(['encouter_id' => $id])->delete();

        foreach($data['bank_soal_id'] as $bank)
        {
            $bankSoal = BankSoal::find($bank);
            $pretest = Encouter::find($id);

            EncouterDetail::create([
                'encouter_id' => $pretest->id,
    ...
    {
        $pretestDetailIds = EncouterDetail::whereBankSoalId($request->bank_soal_id)->pluck('id')->toArray();
        EncouterDetail::destroy($pretestDetailIds);
