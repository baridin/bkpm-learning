...
use App\Pretest;
use App\PretestDetail;
use App\DiklatMataDiklat;
use Carbon\Carbon;

class PretestController extends VController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $data->update($insert);
        // PretestDetail::where('pretest_id',$data['id'])->delete();
        $diklat_mata = DiklatMataDiklat::where('diklat_id',$data['diklat_id'])->inRandomOrder()->first();
        
        if ($request->settings !== 'manual') {
            $listSoal = $data->autoChose($request->options,$diklat_mata->mata_diklat_id);

            foreach ($listSoal as $kls => $vls) {
                foreach ($vls as $klsd => $vlsd) {
                    PretestDetail::create([
                        'pretest_id' => $data['id'],
                        'question' => $vlsd['soal'],
                        'details' => $vlsd['details'],
                        'type_soal' => $vlsd['type_soal'],
        ...
        // PretestDetail::where(['pretest_id' => $id])->delete();
    ...
    public function store(Request $request)
    {
        ...
        $options = [];
        if ($request->setting === 'otomatis') {
            $options = [
                'pg' => $request->pg_count,
                'essay' => $request->essay_count,
            ];
        }
     ...
     public function addBankSoal(Request $request, $id)
    {
        ...
        {
            $bankSoal = BankSoal::find($bank);
            $postest = pretest::find($id);

            PretestDetail::create([
                'pretest_id' => $postest->id,
                'question' => $bankSoal->soal,
                'details' => $bankSoal->details,
                'type_soal' => $bankSoal->type_soal,
                'bank_soal_id' => $bankSoal->id
    ...
    public function addBankSoal1(Request $request, $id)
    {
    ...
    {
        $pretestDetailIds = PretestDetail::whereBankSoalId($request->bank_soal_id)->pluck('id')->toArray();
        PretestDetail::destroy($pretestDetailIds);
