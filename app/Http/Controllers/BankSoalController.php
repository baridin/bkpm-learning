...
class BankSoalController extends VController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->type_soal === 'pg' && $request->has('is_active')) {
            $options = $options->map(function ($item, $key) use($request) {
                if(!isset($request->is_active["{$key}"])) return null;
                else return $item;
            });
        }

        $insertData = array_merge(
            $request->only('mata_diklat_id', 'type', 'type_soal', 'soal'), [
            'details' => $request->type_soal === 'pg' ? json_encode([
                'options' => $options,
    ...
    public function store(Request $request)
    {
        ...
        $data = BankSoal::create($insertData);
    ...
    public function choseSoal(Request $request)
    {
        try {
            $Validator = Validator::make($request->all(), [
                ...
                'type' => [
                    'required',
                    'string',
                    'in:latihan,ujian,pretest,postest'
                ],
                'id' => [
                    'required'
                ]
            ...
            if ($request->action === 'store') {
                $this->storeBankSoal((object)$request->except('_token'));
                return response()->json(["error" => false], 201);
            } elseif ($request->action === 'delete') {
                $this->removeBankSoal((object)$request->except('_token'));
