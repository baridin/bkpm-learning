...
class DiklatDetailController extends VController
{
    ...
    public function store(Request $request)
    {
        ...
        $mataDiklats = $diklat->mataDiklat;
        /* foreach ($mataDiklats as $kmd => $vmd) {
            $lastExec = $vmd->sections()->first()->exercieses()->orderByDesc('line')->first();
            if ($lastExec) {
                $exercise = Exercise::updateOrCreate(
                    ["mata_diklat_id" => $vmd->id, "line" => $lastExec->line + 1],
                    ["title" => "Latihan $vmd->title", "type" => "pg", "section_id" => $lastExec->section_id]
                );
                if ($exercise->id) {
                    $bankExec = BankSoalLatihan::whereMataDiklatId($vmd->id)->get();
                    foreach ($bankExec as $kbx => $vbx) {
                        ExerciseDetail::create([
                            "exercise_id" => $exercise->id,
                            "key" => "soal",
                            "value" => $bankExec->soal,
                            "details" => $bankExec->details,
                        ]);
                    }
                }
            }
        ...
        return redirect()
        ->route("voyager.diklats.edit", $request->diklat_id)
        ->with([
