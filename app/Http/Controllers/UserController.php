...
class UserController extends VController
{
    ...
    public function getKonfirm(Request $request)
    {
        ...
        // })->get();
        $data = DiklatDetailUser::where('status',10)->where('file',null)->get();
        ...
        if ($request->type == 'attach') {
            ...
            $sendMail = Mail::send([], ['name', 'Admin Pendaftaran Diklat E-Learning BKPM'], function ($message) use ($user, $mes) {
                if ($this->request->hasFile('attachment')) {
                 ...
                 $path_tujuan = public_path('konfirm-email');
                 


                $publicPath =  $file->move($path_tujuan,$file->getClientOriginalName());
                    // $publicPath = Storage::putFile('public/mail/konfirm-email', $fullname);
                    // $path = url($publicPath);
                    $message->attach($publicPath);
                }
            ...
            $id_diklat = $request->post('id_diklat');
            $id_diklat_detail = $request->post('id_diklat_detail');

            $diklat_detail_users = new \App\DiklatDetailUser;
            $diklat_users = new \App\DiklatUser;

            $delete_pendaftaran = $diklat_detail_users->where('user_id', $id)
            ->where('diklat_id', $id_diklat)
            ->where('diklat_detail_id', $id_diklat_detail)
            ->update(['status'=>0]);
            ...
            // DB::table('diklat_detail_user_sementara')->insert([
            //     'diklat_id' => $id_diklat,
            //     'diklat_detail_id' => $id_diklat_detail,
            //     'user_id' => $id,
            // ]);

            DB::table('diklat_detail_user_sementara')->where('diklat_id',$id_diklat)
                ->where('diklat_detail_id',$id_diklat_detail)
                ->where('user_id',$id)->delete();
            

            $delete_pendaftaran = $diklat_users->where('user_id', $id)
            ->where('diklat_id', $id_diklat)
            ->delete();

            $sendMail = Mail::send([], ['name', 'Admin Pendaftaran Diklat E-Learning BKPM'], function ($message) use ($user, $mes) {
                $message->to($user->email)
                    ->subject('Permintaan Pendaftaran Anda Kami Tolak.')
                    ->setBody((string)view('vendor.mail.custom-mail')->withMes($mes)->withUser($user), 'text/html');
            });
        }
        $data = [
    ...
    public function postDocument(Request $request, $id, $diklat)
    {
        ...
        if ($request->type == 'attach') {
            ...
            Mail::send([], ['name', 'Admin Pendaftaran Diklat E-Learning BKPM'], function ($m) use ($user, $mes) {
                ...
                $m->to($user->email)
                ->subject('Revisi Dokumen Pendaftaran.')
                ->setBody((string)view('vendor.mail.custom-mail')->withMes($mes)->withUser($user), 'text/html');
            });
        }else{
            ...
            $hitung = DB::table('diklat_detail_user_sementara')->where('user_id', $id)
            ->where('diklat_id', $id_diklat)
            ->where('diklat_detail_id', $id_diklat_detail)
            ->count();
	...
	try {
        Mail::send([], ['name', 'Admin Pendaftaran Diklat E-Learning BKPM'], function ($message) {
            $message->to('risqir57@gmail.com')
                ->subject('Test Permintaan Pendaftaran Anda Kami Tolak. Anda di blacklist!')
                ->setBody('<b>Test</b>', 'text/html');
        });
	} catch (\Exception $e) {
		dd($e);
	}
