...
abstract class Controller extends BaseController
{
    ...
    public function getSlug(Request $request, int $uri=null)
    {
        if (empty($uri)) {
            if (isset($this->slug)) {
                $slug = $this->slug;
            ...
            $slug = explode('.', $request->route()->getName())[$uri];
        }
    ...
    public function insertUpdateData($request, $slug, $rows, $data, $where=NULL)
    {
        $multi_select = [];
        ...
        foreach ($rows as $row) {            
            ...
            // If wistiya hased id
            // if ($row->field == 'wistia_hashed_id' && empty($request->input($row->field))) {
            //     $wi = getWistyaHashedId($data->title);
            //     $data->{$row->field} = $wi;
            // }

            if (get_class($data) == "App\VirtualClass") {
                // on first create zoom
                $virtualClass = VirtualClass::find($data->id);
                if ($row->field == 'zoom_id') {
                    if (empty($request->input($row->field))) {
                        $zoom = $this->createZoomRoom($data, $request);
                        // insert zoom_id
                        $data->{$row->field} = $zoom['detail']['id'];
                    } else {
                        if (!empty($virtualClass->id) && ($request->input('zoom_account_id') !== $virtualClass->zoom_account_id)) {
                            // if current zoom_account_id != previous account
                            // Delete current meeting
                            deleteZoom($virtualClass->zoom_id, $virtualClass->type, $virtualClass->zoom_account_id);
                            $zoom = $this->createZoomRoom($data, $request);
                            // insert zoom_id
                            $data->{$row->field} = $zoom['detail']['id'];
                        }
            ...
            if ($row->field == 'zoom_join' && empty($request->input($row->field))) {
                $data->{$row->field} = $zoom['detail']['join_url'];
            }
            
            if ($row->field == 'zoom_start' && empty($request->input($row->field))) {
                $data->{$row->field} = $zoom['detail']['start_url'];
            }

            if ($row->field == 'line' && empty($request->input($row->field))) {
                $data->{$row->field} = $data->where($where)->get()->count() + 1;
            }

            if (get_class($data) == 'App\DiklatDetail') {
                $data->title = 'Angkatan '.integerToRoman($data->force);
            }
        }
        ...
        foreach ($multi_select as $sync_data) {
            if ($sync_data['table'] == 'diklat_parrents') {
                $data->belongsToMany($sync_data['model'], $sync_data['table'], 'diklat_id', 'parent_id')->sync($sync_data['content']);
            } else {
    ...
    public function createZoomRoom($data, $request)
    {
        $body = [
            "topic" => "{$data->title}",
            "type" => ($data->type == 'meeting')?3:6, // 6 for webinar, 3 form meeting
        ...
        return createZoom($body, $data->type, (int)$request->input('zoom_account_id'));
    }
}
