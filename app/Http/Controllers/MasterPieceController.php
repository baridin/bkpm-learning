...
class MasterPieceController extends VController
{
    ...
    public function store(Request $request)
    {
        ...
        // Check permission
        $this->authorize('add', app($dataType->model_name));
        $matadiklat = MataDiklat::where('id','=',$request->mata_diklat_id)->first();
        $diklat = Diklat::where('id','=',$request->diklat_id)->first();
        $user = User::where('id','=',$request->user_id)->first();
        $get_diklat_title = Diklat::find($diklat->id);
        $get_mata_diklat_title = DiklatDetail::find($request->mata_diklat_id);
            // Validate fields with ajax
        $val = $this->validateBread($request->all(), $dataType->addRows)->validate();
        $data = $this->insertUpdateData($request, $slug, $dataType->addRows, new $dataType->model_name());
        $mail = $user->email;
        $mes = "Silahkan mengerjakan tugas dari instruktur dibawah ini.";
        $subject =  "Tugas $get_diklat_title->title - $matadiklat->title untuk $user->name";
        $pdf = $request->file('pdf');
        event(new BreadDataAdded($dataType, $data));
         Mail::send([], ['name', 'Permintaan Approve Didital Signature'], function ($message) use ($mail, $mes,$subject,$pdf) {
                    $message->to($mail)
                    ->subject($subject)
                    ->attachData($pdf,'file.pdf')
