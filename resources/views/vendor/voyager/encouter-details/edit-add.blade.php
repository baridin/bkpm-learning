                        ...
                        <div class="panel-body">
                            ...
                            @foreach($dataTypeRows as $row)
                                ...
                                <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                    ...
                                    @else
                                        @if ($row->field === 'encouter_id' && request()->has('encouter_id'))
                                            <input type="hidden" name="{{$row->field}}" class="form-control" value="{{request()->get($row->field)}}">
                                        @else
                                    ...
                                    <div id="detail-options">
                                                ...
                                                @if ($k === 'options')
                                                    @foreach ($options as $ko => $vo)
                                                        ...
                                                        <div class="form-group  col-md-12 ">
                                                            <label class="control-label" for="name">Pilihan {{strtoupper($vo)}}</label>
                                                            <textarea class="form-control" name="options[{{$vo}}]" rows="3" placeholder="Pilihan {{$vo}}">{{$val}}</textarea>
                                                        </div>
                                                    @endforeach
                                                @endif
                                            @endforeach
                                        @else
                                            @foreach ($options as $op)
                                                <div class="form-group col-md-12 ">
                                                    <label class="control-label" for="name">Pilihan {{strtoupper($op)}}</label>
                                                    <textarea class="form-control" name="options[{{$op}}]" rows="3" placeholder="Pilihan {{$op}}"></textarea>
                                                </div>
                                            @endforeach
                                        @endif
                                        @if (!empty($dataTypeContent->{$row->field}))
