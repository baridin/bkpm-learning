        ...
        <div class="col-md-12">
            @if ($edit)
                <div id="bank-soal" class="panel panel-bordered">
                    ...
                    <div class="panel-body">
                        @php $getMataDiklat = App\DiklatMataDiklat::where('diklat_id', $dataTypeContent->diklat_id)->get(); @endphp
                        @foreach ($dataTypeContent->bankSoal($dataTypeContent->mata_diklat_id) as $kbk => $vbk)
                        @if(strtoupper($kbk) == 'PRETEST_POSTEST')
                            <div class="collapsible">

                                <div class="collapse-head" data-toggle="collapse" data-target="#{{$kbk}}" aria-expanded="true" aria-controls="{{$kbk}}">
                                    <h4>Postest    <span class="label label-primary">{{$dataTypeContent->countBankSoal($kbk)}}</span></h4>
                                    <i class="voyager-angle-down"></i>
                                    <i class="voyager-angle-up"></i>
                                </div>
                                <div class="collapse-content collapse in" id="{{$kbk}}">
                                  {{--   <button class="btn btn-success" onclick="proses('{{$kbk}}')">Pilih Soal Postest</button>
 --}}
                                    <div class="table-responsive">
                                        <table class="table table-hover dataTable no-footer">
                                            ...
                                            <tbody>
                                                @foreach($getMataDiklat as $gmd)
                                                @foreach ($vbk as $kbkd => $vbkd)
                                                 @if($vbkd->mata_diklat_id == $gmd->mata_diklat_id)
                                                    <tr>
                                                         @php
                                                            $is_ready = $dataTypeContent->chosenBankSoalId($vbkd->id); 
                                                            $url = !$is_ready ? route('voyager.postest-bank-soal.store', $dataTypeContent->id) : route('voyager.postest-bank-soal.delete', $dataTypeContent->id); /*delete*/
                                                        @endphp
        ...
        @if($edit)
             ...
             function check(id, en, type)
            {
                ...
                data = PRETEST_POSTEST
                 ...
                 $.ajax({
                    method: 'post',
                    url: '{{route('voyager.postest-bank-soal.store', $dataTypeContent->id)}}',
                    data: {
                        bank_soal_id: data,
                    },
                    success(data)
