...
@section('content')
    <div class="page-content edit-add container-fluid">
        <div class="row">
                ...
                <div class="panel panel-bordered">
                    ...
                    <form role="form"
                        ...
                        <div class="panel-body">
                            ...
                            @endforeach
                            <div class="form-group  col-md-12" id="ini_video"> 
                                <label class="control-label" for="name">Video</label> 
                                <input type="file" name="video_youtube" > </div>
                            <div class="form-group  col-md-12 " id="ini_pdf"> <label class="control-label" for="name">File Pdf</label> <input type="file" name="file[]" multiple="multiple"> </div>

                        </div><!-- panel-body -->
                                <input type="hidden" name="mt_id" value="{{ Request::get('mt_id') }}">
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    @include('vendor.voyager.materials.quizz', [
                        'dataType' => $dataQuizz,
                        'dataTypeContent' => $dataTypeContentQuizz
                    ])
                </div>
            </div>
            @endif
...
@section('javascript')
    ...
    <script>
        var video = $('#option-type-video').val();
        if(video == 'video'){
            $('#ini_video').show();
            $('#ini_pdf').hide();
        }else{
        ...
        $('#option-type-video').on('click',function(){
            $('#ini_video').show();
            $('#ini_pdf').hide();
        })
        $('#option-type-pdf').on('click',function(){
            $('#ini_video').hide();
            $('#ini_pdf').show();
        ...
        $(function(){
            // Wistya upload file
            const WT = `{!!config('services.wistia.token')!!}`
            window._wapiq = window._wapiq || [];
            _wapiq.push(function (W) {
                window.wistiaUploader = new W.Uploader({
                    accessToken: "cd3b4551c928318eec5bdbc6165adc392c3d76dd6f5015120fe042e3e51b3a78",
                    button: "wistia_upload_button_button_only",
                    projectId: "6a23sqoftq"
                });
                wistiaUploader.bind('uploadsuccess', function(file, media) {
                    // console.log(file, media, media.thumbnail.url);
                    $('input[name="wistia_hashed_link"]').val(media.id)
                    // $('input[name="thumbnail"]').val(media.thumbnail.url)
                })
            });
    ...
    <script>
        ...
        $('td').on('click', '.delete', function (e) {
            $('#delete_form')[0].action = '{{ route('voyager.quizzs.destroy', ['id' => '__id']) }}'.replace('__id', $(this).data('id'));
