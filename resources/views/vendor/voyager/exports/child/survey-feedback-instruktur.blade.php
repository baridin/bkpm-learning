@foreach ($datas->search->relation['getDiklat'] as $d)
    ...
    @foreach ($diklat->diklatDetail as $dd)
        ...
        @php $no = 0; @endphp
        @php $get_instruktur = App\DiklatInstruktur::where('diklat_id',$dd->diklat_id)->get();  @endphp
        @foreach ($get_instruktur as $inst)
        @php $get_nama = App\Instruktur::where('id',$inst->instruktur_id)->first(); @endphp
            <h3>Instruktur : {{ $get_nama->name }}</h3>
            <br>
            @php $number = 1; @endphp
            @foreach ($diklat->survey_instruktur() as $dm)
            @php $cek = App\SurveyFeedbackInstrukturUser::where('diklat_id',$dd->diklat_id)->where('instruktur_id',$inst->instruktur_id)->first(); @endphp
            @if($cek)
                <table class="paleBlueRows">
                    <thead>
                        <tr>
                            <th><b>Survey: {{$number++}}). {{$dm->question}}</b></th>
                        </tr>
                        <tr>
                            <th>No</th>
                        ...
                        @foreach($dd->users(1)->get() as $data)
                            <tr>
                                ...
                                @foreach ($datas->dataType->browseRows as $row)
                                    ...
                                    @endif
                                @endforeach
                                <td>{{round($data->mySurveyso($data->getKey(), $diklat->getKey(), $dm->getKey(),$get_nama->id ))}}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
