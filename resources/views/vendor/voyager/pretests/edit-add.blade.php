                    ...
                    <div class="panel-body">
                        ...
                        @foreach($dataTypeRows as $row)
                            ...
                            <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                {{ $row->slugify }}
                                @if($row->display_name !== "Mata Diklat Id")
                        ...
                        @if(strtoupper($kbk) == 'PRETEST_POSTEST')
                            <div class="collapsible">
                                <div class="collapse-head" data-toggle="collapse" data-target="#{{$kbk}}" aria-expanded="true" aria-controls="{{$kbk}}">
                                    <h4>Pretest    <span class="label label-primary">{{$dataTypeContent->countBankSoal($kbk)}}</span></h4>
                                                ...
                                                @foreach ($vbk as $kbkd => $vbkd)
                                                    ...
                                                    <tr>
                                                         ...
                                                         @php
                                                            ...
                                                            $url = !$is_ready ? route('voyager.pretest-bank-soal.store', $dataTypeContent->id) : route('voyager.pretest-bank-soal.delete', $dataTypeContent->id); /*delete*/
                                                        @endphp
           ...
           function check(id, en, type)
            {
                ...
                data = PRETEST_POSTEST
                 ...
                 $.ajax({
                    method: 'post',
                    url: '{{route('voyager.pretest-bank-soal.store', $dataTypeContent->id)}}',
                    data: {
                        bank_soal_id: data,
                    },
                    success(data)
