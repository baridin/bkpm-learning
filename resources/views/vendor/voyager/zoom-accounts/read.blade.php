            ...
            <div>
                ...
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation"><a href="#profile" aria-controls="profile" role="tab"
                            data-toggle="tab">Virtual Class
                            <span class="label label-warning">{{$virtualMeeting->count()}}</span></a></li>
                    <li role="presentation"><a href="#messages" aria-controls="messages" role="tab"
                            data-toggle="tab">Virtual Webinar
                            <span class="label label-warning">{{$virtualWebinar->count()}}</span></a></li>
                </ul>
                ...
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane active" id="home">
                        <div class="panel panel-bordered" style="padding-bottom:5px;">
                            ...
                            <div class="panel-body" style="padding-top:0;">
                                ...
                                => $dataTypeContent, 'content' => $dataTypeContent->{$row->field}, 'action' => 'read'])
                                ...
                                @else
                                @include('voyager::multilingual.input-hidden-bread-read')
                                @if ($row->field == 'zoom_join' || $row->field == 'zoom_start')
                                <a href="{{ $dataTypeContent->{$row->field} }}" class="btn btn-warning"
                                    target="_blank">{{($row->field == 'zoom_join')?'Bergabung':'Mulai'}}</a>
                                @else
                                <p>{{ $dataTypeContent->{$row->field} }}</p>
                                @endif
                                @endif
                    ...
                    <div role="tabpanel" class="tab-pane" id="profile">
                        <div class="panel panel-bordered" style="padding-bottom:5px;">
                            <div class="panel panel-bordered" style="padding-bottom:5px;">
                                ...
                                @forelse ($virtualMeeting as $km => $vm)
                                <div class="panel-heading" style="border-bottom:0;">
                                    <div style="display: flex; align-content: center; align-items: center;">
                                        <h3 class="panel-title" style="flex: 1;">{{$vm->title}}</h3>
                                        <a href="{{ route('voyager.virtual-classes.show', $vm->id) }}"
                                            class="btn btn-warning" style="margin: 0 25px;" title="{{ 'show' }}">
                                            <span class="hidden-xs hidden-sm">{{ 'Lihat' }}</span>
                                        </a>
                                    </div>
                                </div>

                                <div class="panel-body" style="padding-top:0;">
                                    <p><b>Meeting Id : {{$vm->zoom_id}}</b></p>
                                    <p><b>Tgl Mulai : {{Carbon\Carbon::parse($vm->start_at)->format('d M Y')}}</b></p>
                                </div><!-- panel-body -->
                                <hr style="margin:0;">
                                @empty
                                <div class="panel-heading" style="border-bottom:0;">
                                    <div style="display: flex; align-content: center; align-items: center;">
                                        <h3 class="panel-title" style="flex: 1;">Data belum tersedia</h3>
                                    </div>
                                </div>
                                @endforelse
                            </div>
                    ...
                    <div role="tabpanel" class="tab-pane" id="messages">
                        <div class="panel panel-bordered" style="padding-bottom:5px;">
                            @forelse ($virtualWebinar as $km => $vm)
                            <div class="panel-heading" style="border-bottom:0;">
                                <div style="display: flex; align-content: center; align-items: center;">
                                    <h3 class="panel-title" style="flex: 1;">{{$vm->title}}</h3>
                                    <a href="{{ route('voyager.virtual-classes.show', $vm->id) }}" class="btn btn-warning"
                                        style="margin: 0 25px;" title="{{ 'show' }}">
                                        <span class="hidden-xs hidden-sm">{{ 'Lihat' }}</span>
                                    </a>
                                </div>
                            </div>
