                            ...
                            @foreach($dataTypeRows as $row)
                                ...
                                <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                    ...
                                    @elseif ($row->type == 'relationship')
                                        @if ($row->field == 'virtual_class_belongsto_diklat_detail_relationship')
                                        ...
                                        @elseif ($row->field == 'virtual_class_belongsto_mata_diklat_relationship')
        ...
        $('document').ready(function () {
            $('select[name="diklat_id"]').on('change', (e)=>{
                ...
                if (optionSelected.val() != '') {
                    $.ajax({
                        method: 'GET',
                        url: `{!!url('admin/virtual-classes/relation')!!}?diklat_id=${optionSelected.val()}&type=get_by_diklat&method=edit`,
                        dataType: 'json',
                        success: (res)=>{
                            // console.log(res);
                            let html_mata = ``
