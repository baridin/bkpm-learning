                ...
                <div class="panel panel-bordered" style="padding-bottom:5px;">
                    ...
                    @foreach($dataType->readRows as $row)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @else
                                @include('voyager::multilingual.input-hidden-bread-read')
                                @if ($row->field == 'details' && !empty($dataTypeContent->{$row->field}))
                                    @forelse (json_decode($dataTypeContent->{$row->field})->options as $key => $item)
                                        <p><b>{{strtoupper($key)}}</b> : {!!!empty($item) ? $item : 'empty'!!}</p>
                                    @empty
                                    @endforelse
                                    <p><b>Jawaban Benar</b> : {!!strtoupper(json_decode($dataTypeContent->{$row->field})->is_true)!!}</p>
                                @else
                                    <p>{{ $dataTypeContent->{$row->field} }}</p>
                                @endif
                            @endif
