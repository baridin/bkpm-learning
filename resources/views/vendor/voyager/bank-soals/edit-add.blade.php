...
@section('javascript')
    ...
    <script>
        ...
        $('document').ready(function () {
            ...
            $('input[type=radio][name=type_soal]').change(function() {
                if (this.value == 'essay') {
                    ...
                    $('#detail-options').show()
                    mapOptionElements(this.value)
                }
            });
        });

        function mapOptionElements(type) {
            const options = ['a', 'b', 'c', 'd', 'e'];
            options.forEach(el => {
                const listen = $(`#is_active_${el}`)
                if (type === 'pg') { listenElements(listen.prop('checked'), el); }
                listen.change(function() {
                    if ($(this).prop('checked')) { showElements(el); }
                    else { hideElements(el); }
                })
            });
        }
        ...
        function showElements(el) {
            ...
            $(`#textarea_${el}`).show()
            $(`#textarea_${el}`).prop('required', true);
            return
        }
        ...
        function hideElements(el) {
            ...
            $(`#textarea_${el}`).hide()
            $(`#textarea_${el}`).prop('required', false);
            return
        }

        function listenElements(isActive, id) {
            if (isActive) { showElements(id); }
            else { hideElements(id); }
        }
    </script>
@stop
