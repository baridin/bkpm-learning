...
@section('page_title', __('voyager::generic.viewing').' Review User')

@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            <i class="voyager-params"></i> Review {{(isset($title))?ucwords($title):'Ujian'}}
        </h1>
    </div>
@stop
                        ...
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    @php
                                        $get_remedial = App\EncouterUser::where('user_id',$iv['user_id'])->where('encouter_id',$iv['ujian_id'])->first();
                                    @endphp
                                    @if($get_remedial)
                                        <tr>
                                            <td>{{++$i}}</td>
                                            ...
                                            <td>{{$iv['users']}}</td>
                                            <td>
                                                @if($get_remedial->is_remedial !== 1)
                                                @if(isset($type))
                                                    @if($type == 'udah')
                                                        <a
                                                        href="#"
                                                        onclick="deletes('{{url('admin/delete-assesment')}}?enc_id={{$iv['id']}}&usr_id={{$iv['user_id']}}')"
                                                        class="btn btn-danger btn-add-new pull-right">
                                                            <i class="voyager-trash"></i> <span>{{ __('Hapus') }}</span>
                                                        </a>
                                                        <a
                                                        @if (isset($title))
...
@section('javascript')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    @endphp
                    <form action="/admin/get-review-assesment" class="text-left">
                        <div class="form-group">
    ...
    <script>
        ...
        {
            let check = confirm('Apakah anda yakin ?')
