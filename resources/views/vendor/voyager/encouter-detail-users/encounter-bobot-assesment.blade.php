...
@section('page_header')
    <h1 class="page-title">
        <i class="{{ __('voyager-eyes') }}"></i> {{ __('Menilai Diklat').' '.ucwords($use->name) }}&nbsp;
...
@section('content')
<form id="post-assesment" action="{{url('admin/post-encounter-bobot-assesment')}}" method="post" enctype="multipart/form-data">
    @csrf
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-bordered" style="padding-bottom:5px;">
                    ...
                    <input type="hidden" name="diklat_id" value="{{$diklat->id}}">
                    @foreach($datas as $kr => $row)
                        ...
                        <div class="panel-footer">
                            <div class="input-group input-group-sm">
                                ...
                                <input type="hidden" name="bobot_id" value="{{$row['detail_id']}}">
                                <input type="number" name="value_{{$row['detail_id']}}" max="100" class="form-control" placeholder="Nilai" value="{{$row['nilai']}}" aria-describedby="sizing-addon{{$kr}}">
                            </div>
                        </div>
                        <hr style="margin:0;">
                    @endforeach
